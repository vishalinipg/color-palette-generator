...
function App() {
    ...
    const generateColorPalette = () => {
        ...
        for (let i = 0; i < maxColorBoxes; i++) {
            const randomHexColor = `#${Math.floor(Math.random() * 0xffffff)
                .toString(16)
                .padStart(6, "0")}`;
            newColorList.push(randomHexColor);
        }
    ...
    const copyColorToClipboard = (hexValue, index) => {
        navigator.clipboard
            .writeText(hexValue)
            .then(() => {
    ...
    const handleSearchChange = (e) => {
        ...
        const matched = colorMapping[input] || [];
        setMatchingColors(matched);
    };

    const filteredColorList = matchingColors.length > 0 ? matchingColors : colorList;
    ...
    return (
        <div>
            ...
            <ul className="container">
                {filteredColorList.map((hexValue, index) => (
                    <li
                        className="color"
                        key={index}
                        onClick={() => copyColorToClipboard(hexValue, index)}
                    >
                        <div
                            className="rect-box"
            ...
            <button className="refresh-btn" onClick={generateColorPalette}>
                Refresh Palette
            </button>
        </div>
    );
